...
KAS_MACHINE = p3768-0000-p3767-0004
KAS_DISTRO ?= poky
...
DOCKER_IMAGE_PRIMARY ?= image_jetson
DOCKER_IMAGE_FALLBACK ?= image-jetson
DOCKER_WORKDIR ?= /work
...
list-images:
	...
	@find build/tmp/deploy/images -type f -name "*.wic" -o -name "*.sdimg" -o -name "*.jetson-sdimg" | sort
...
copy-artifacts:
	...
	@if [ -d "build/tmp/deploy/images/$(KAS_MACHINE)" ]; then \
		...
		$(CP) build/tmp/deploy/images/$(KAS_MACHINE)/*.jetson-sdimg $(ARTIFACTS_DIR)/images/ 2>/dev/null || true; \
